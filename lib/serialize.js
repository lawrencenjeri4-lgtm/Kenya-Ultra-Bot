...
// Fetch bot info directly
let botAdmin = false;

try {
    const groupInfo = await sock.groupFetchAllParticipating();
    const currentGroup = groupInfo[m.chat];

    if (currentGroup?.participants) {
        const botParticipant = currentGroup.participants.find(
            p => p.id === botJid
        );

        botAdmin = !!botParticipant?.admin;
    }
} catch (e) {
    console.log("groupFetchAllParticipating failed");
}
...
console.log("==================================");
            ...
            const normalize = (jid = "") => {
    return jid
        .split("@")[0]      // Remove @s.whatsapp.net or @lid
